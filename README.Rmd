...
```{r, pp_plot1, fig.width = 8, eval = FALSE}
pp_plot(reading ~ ethnicity, benchmarks)
```
![pp_plot1](inst/image/README-pp_plot1-1.png)
...
```{r dev_off, include = FALSE}
par(mfrow = c(1, 1))
```

```{r pp_plot2, fig.width = 8, eval = FALSE}
pp_plot(reading ~ frl, benchmarks)
```
![pp_plot2](inst/image/README-pp_plot2-1.png)
